...
async function findElement(p: Page, selectors: string[]) {
...
const server = new Server(
  { name: "woolworths-nz", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
  ...
  tools: [
    {
      ...
      description:
        "Log in to your Woolworths NZ account. Call this first each session. Credentials can also be pre-set via WOOLWORTHS_EMAIL / WOOLWORTHS_PASSWORD environment variables.",
      inputSchema: {
        type: "object",
        properties: {
  ...
  try {
    switch (name) {
        ...
        const products = await p.evaluate(
          (lim, specialOnly) => {
            ...
            return tiles
              .map((tile) => {
                const nameEl =
                  tile.querySelector('[class*="ProductName"]') ??
                ...
                const idSources = [
                  ...
                  (tile.querySelector('button[class*="add" i]') as HTMLElement | null)?.dataset?.productId,
      ...
      case "woolworths_add_to_cart": {
        ...
        if (!productId) return { content: [{ type: "text", text: "productId is required" }] };
        ...
        const cartCount = await p.evaluate(() => {
          const el =
            document.querySelector('[data-testid="cart-quantity"]') ??
      ...
      case "woolworths_view_cart": {
        ...
        const cart = await p.evaluate(() => {
          ...
          const parsed = items.map((item) => {
            const name =
              item.querySelector('[class*="ProductName"]')?.textContent?.trim() ??
              item.querySelector('[class*="product-name"]')?.textContent?.trim() ??
              item.querySelector("h3, h4")?.textContent?.trim() ??
              "Unknown";

            const price =
              item.querySelector('[class*="Price"]:not([class*="Was"])')?.textContent?.trim() ?? "";

            const qty =
              (item.querySelector('input[type="number"]') as HTMLInputElement | null)?.value ??
        ...
        const lines = cart.items.map(
          (item, i) => `${i + 1}. ${item.name} × ${item.qty}  ${item.price}${item.id ? `  [ID: ${item.id}]` : ""}`
        );
      ...
      case "woolworths_remove_from_cart": {
        ...
        const removed = await p.evaluate((pid) => {
          ...
          const removeBtn =
            row.querySelector<HTMLButtonElement>('button[aria-label*="Remove" i]') ??
            row.querySelector<HTMLButtonElement>('[class*="Remove"]') ??
            row.querySelector<HTMLButtonElement>('[data-testid*="remove"]');
      ...
      case "woolworths_get_specials": {
        ...
        const specials = await p.evaluate((lim) => {
          ...
          return tiles.slice(0, lim).map((tile) => {
            const name =
              tile.querySelector("[class*='ProductName']")?.textContent?.trim() ??
              tile.querySelector("h3, h4")?.textContent?.trim() ??
              "";
            const price =
              tile.querySelector("[class*='PriceValue']")?.textContent?.trim() ??
              tile.querySelector("[class*='Price']:not([class*='Was'])")?.textContent?.trim() ??
              "";
            const wasPrice =
              tile.querySelector("[class*='WasPrice']")?.textContent?.trim() ??
